...
import image from './liked.svg';
import heart from './shape.svg';
import { UI_ELEM } from './view';
import { tab } from './tab';
...
UI_ELEM.FORM.addEventListener('submit', (event) => {
  ...
  UI_ELEM.NOWS__LIKE.setAttribute('src', `${heart}`);
...
function createNewElement(city) {
  ...
  cityElement.addEventListener('click', () => {
    UI_ELEM.NOWS__LIKE.setAttribute('src', `${image}`);
    UI_ELEM.INPUT_NAME.classList.remove('error');
...
function setCookie(cityName) {
  ...
  Cookies.set('city', cityName, { expires: inOneHour });
}
