...
function setWeatherData(weatherData) {
	...
	UI_ELEM.DETAILS_SUNRISE.textContent = `Sunrise: ${weatherData.sunrise}`;
	UI_ELEM.DETAILS_SUNSET.textContent = `Sunset: ${weatherData.sunset}`;
}
...
function createTables(forecastList) {
	...
	for (let element of forecastList) {
		...
		td1.textContent = element.dt_txt.slice(0, 10);
